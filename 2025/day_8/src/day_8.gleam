...
fn combine_n_circults(
  n: Int,
  distances: List(#(Int, Junction, Junction)),
  circuts: List(Circut),
) {
  list.index_fold(distances, circuts, fn(cs, distance, i) {
    case i < n {
      True -> {
        let #(_, j_a, j_b) = distance
        case combine_juction_circuts(j_a, j_b, cs) {
          Ok(new_curcuts) -> {
            new_curcuts
          }
          Error(_) -> cs
...
pub fn main() -> Nil {
  ...
  combine_n_circults(1000, distances, circuts)
