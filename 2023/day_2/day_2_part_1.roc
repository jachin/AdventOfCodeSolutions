...
main =
    ...
    when finalResult is
        ...
        Ok fileContentStr ->
            ...
            possibleGames = List.dropIf
                games
                (\game ->
                    List.any game.draws (\draw -> draw.red > 12 || draw.green > 13 || draw.blue > 14)
                )

            answer =  List.map possibleGames .number |> List.sum |> Num.toStr
...
Game : { number : I32, draws : List Draw }
...
parseGame = \inputStr ->
    ...
    when parts is
        ...
        [_] -> Err "No game found"
        [label, data] ->
            Ok { number: parseGameNumber label, draws: parseDraws data }
